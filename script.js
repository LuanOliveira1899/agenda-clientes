...
async function validatePhoneNumber(phone) {
  const apiKey = "9f131cf462fd02dba29816d031fc0d8b";
  const formattedPhone = phone.replace(/\D/g, "");

  try {
    const response = await fetch(
      `https://apilayer.net/api/validate?access_key=${apiKey}&number=55${formattedPhone}`
    );
    ...
    console.error(error);
    alert(
      "Erro ao validar telefone, pois o serviço não está funcionando corretamente"
    );
    return false;
  }
}
...
function exportToExcel() {
  ...
  XLSX.utils.sheet_add_aoa(worksheet, [["Nome", "Telefone", "Email"]], {
    origin: "A1",
  });
